...
class Gaussian(nn.Module):
    ...
    def compute_kl(self, other):
        ...
        kl_div = (torch.mul(term1 + term2 + term3 - 1, 0.5)).sum().item()
        return kl_div
...
class ProbCNNet9lChannel(nn.Module):
    ...
    ----------
    l_0 : int
        Position to insert the wireless channel. Set to -1 to disable.
        0: Input, 1: After conv1, 2: After conv2, 3: After pool1,
        4: After conv3, 5: After conv4, 6: After pool2,
        7: After conv5, 8: After conv6, 9: After pool3,
        10: After fcl1, 11: After fcl2, 12: After fcl3 (output).
...
def trainPNNet(net, optimizer, pbobj, epoch, train_loader, lambda_var=None, optimizer_lambda=None, verbose=False):
    ...
    return avgbound/batch_id, avgkl/batch_id, avgloss/batch_id, avgerr/batch_id
...
def testStochastic(net, test_loader, pbobj, device='cuda'):
    ...
    with torch.no_grad():
        for batch_id, (data, target) in enumerate(tqdm(test_loader)):
            ...
            # why for loop over each data point?
